...
FiniteDifference::FiniteDifference(double S, double K, double T, double sigma, double r, double q, int time_step, double alpha, bool eu) : euro(eu),m_S0(S), m_K(K), m_T(T), m_sigma(sigma), m_r(r), m_q(q), M(time_step), alpha_tmp(alpha)
{
...
{
	return m_K * exp(a * x_left + b * tau) * (1 - exp(x_left));
}
...
void FiniteDifference::Forward_Euler()
{
	...
	for (int t = 1; t <= M; t++)
	{
		...
		for (int i = 1; i < N; i++)
		{
			...
			if (this_level[i] < early_ex)
				this_level[i] = early_ex;
		}
		Results.push_back(this_level);
	}
