...
FiniteDifference::FiniteDifference(double S, double K, double T, double sigma, double r, double q, int time_step, double alpha, bool europe) : euro(europe), m_S0(S), m_K(K), m_T(T), m_sigma(sigma), m_r(r), m_q(q), M(time_step), alpha_tmp(alpha)
{
...
void FiniteDifference::Crank_Nicolson()
{
	...
	for (int t = 1; t <= M; t++)
	{
		std::vector<double> last_level(this_level);
		this_level[0] = g_left(tau_grid[t]);
		if (this_level[0] < m_K * exp(a * x_left + b * t * delta_tau) * (1 - exp(x_left)))
			this_level[0] = m_K * exp(a * x_left + b * t * delta_tau) * (1 - exp(x_left));
		this_level[N] = g_right(tau_grid[t]);
		vec rhs(N - 1);
		vec early_exercise(N - 1);
		for (int i = 0; i < N - 1; i++)
		{
			...
			double tau = t * delta_tau;
			double early_ex = m_K * exp(a * x + b * tau) * (1 - exp(x));
			if ((1 - exp(x)) < 0 || euro)
				early_ex = 0;
			early_exercise[i] = early_ex;
		}
		
		auto res = ls.sor_banded(early_exercise,A,rhs,early_exercise,1e-6,1.15,1,1,consecutive) ;
