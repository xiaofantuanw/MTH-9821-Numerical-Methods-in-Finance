...
BinomialBSTree::BinomialBSTree(double S, double K, double T, double sigma, double r, double q, int n, bool call, bool eu) : m_S0(S), m_K(K), m_T(T), m_sigma(sigma), m_r(r), m_q(q), N(n-1), Call(call), Eu(eu)
{
	dt = m_T / n;
...
void BinomialBSTree::Simulate_Tree()
{
	...
	for (int i = 0; i <= N; i++)
	{
		double curr_price = m_S0 * pow(u, N - i) * pow(d, i);
